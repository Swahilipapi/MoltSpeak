...
app.put('/api/v1/agents/:id', (req, res) => {
  try {
    const { id } = req.params;
    const { endpoint, description, capabilities, public_key } = req.body;
    ...
    // TODO: Verify signature with public key to authorize update
  ...
  try {
    ...
    res.json({
      ...
      pagination: {
        ...
        limit,
        offset,
        has_more: offset + agents.length < count
      }
    });
...
// Agent heartbeat (marks as alive)
